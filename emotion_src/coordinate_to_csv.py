...
class Coordinate(object):
        ...
        for file_arg, file_path in enumerate(file_paths):
            ...
            if landmarks is not None:
                ...
                if landmarks.shape  == (1, 68, 2):
                    ...
                    result = dist_angle(landmarks)
                    store_str = array_to_str(result)
    ...
    def _load_KDEF(self):
        ...
        for file_arg, file_path in enumerate(file_paths):
            ...
            image_array = io.imread(file_path)
            landmarks = fa_3D.get_landmarks(image_array)
            if landmarks is not None:
                landmarks = np.asarray(landmarks)
                if landmarks.shape == (1, 68, 3):
                    landmarks = np.squeeze(landmarks)
                    result = dist_angle(landmarks)
                    store_str = array_to_str(result)
                    details['distarray'] = store_str
...
def dist_angle(landmarks):
    xlist = []
    ylist = []
    # zlist = []
    for i in range(len(landmarks)):
        xlist.append(float(landmarks[i][0]))
        ylist.append(float(landmarks[i][1]))
        # zlist.append(float(landmarks[i][2]))
    xmean = np.mean(xlist)
    ymean = np.mean(ylist)
    # zmean = np.mean(zlist)
    xcentral = [(x-xmean) for x in xlist]
    ycentral = [(y-ymean) for y in ylist]
    coormean = np.asarray((xmean, ymean))
    result = []
    for x, y, c1, c2 in zip(xlist, ylist, xcentral, ycentral):
        result.append(x)
        result.append(y)
        # result.append(z)
        coordinate = np.asarray((x, y))
        dist = np.linalg.norm(coordinate - coormean)
        result.append(dist)
        # angle = math.acos((x*xmean + y*ymean + z*zmean) /
        #                  (np.sqrt(np.sum(coordinate**2) * np.sum(coormean**2))))
        # angle = (angle * 360) / (2 * math.pi)
        angle = (math.atan2(c2, c1) * 360) / (2 * math.pi)
        result.append(angle)
...
def array_to_str(dist_angle):
    dist_angle = dist_angle.flatten()
    store_str = ''
    for i in dist_angle:
...
def write_to_csv(headers, rows, dataset_name):
    ...
    csv_path = os.path.join(save_path, 'dist_angle_2D.csv')
